...
// void DetectCone::mobilenet(std::string imgPath, float confidenceThreshold){
//   const size_t inWidth = 300;
//   const size_t inHeight = 300;
//   // const char* classNames[] = { "background", "blue", "yellow", "orange", "orange2" };

//   std::string weights = "frozen_inference_graph.pb";
//   std::string prototxt = "ssd_mobilenet_v1_coco.pbtxt";
...
//           if(objectClass == 1)
//               cv::rectangle(rectified, object, cv::Scalar(255, 0, 0),1);
//           else if(objectClass == 2)
//               cv::rectangle(rectified, object, cv::Scalar(0, 255, 255),1);
//           else if(objectClass == 3)
//               cv::rectangle(rectified, object, cv::Scalar(0, 165, 255),1);
//           else if(objectClass == 4)
//               cv::rectangle(rectified, object, cv::Scalar(0, 0, 255),2);
//       }
//   }
...
//   // cv::imshow("image", rectified);
//   // cv::waitKey(0);

// }

void DetectCone::forwardDetectionRoI(const std::string& img_path, tiny_dnn::network<tiny_dnn::sequential> m_slidingWindow){
  ...
  if(inputs.size()>0){
    ...
    for(size_t i = 0; i < inputs.size(); i++){
      ...
      if (maxIndex == 0 || maxProb < threshold){
        std::cout << "No cone detected" << std::endl;
        cv::circle(img, cv::Point (x,y), circle_radius, cv::Scalar (0,0,0), -1);
      } 
      else{
        std::cout << "Find one " << labels[maxIndex-1] << " cone"<< std::endl;
        if (labels[maxIndex-1] == "blue")
          cv::circle(img, cv::Point (x,y), circle_radius, cv::Scalar (255,0,0), -1);
        else if (labels[maxIndex-1] == "yellow")
          cv::circle(img, cv::Point (x,y), circle_radius, cv::Scalar (0,255,255), -1);
        else if (labels[maxIndex-1] == "orange")
          cv::circle(img, cv::Point (x,y), circle_radius, cv::Scalar (0,165,255), -1);
        else if (labels[maxIndex-1] == "big orange")
          cv::circle(img, cv::Point (x,y), circle_radius*2, cv::Scalar (0,0,255), -1);
      }
    }
  }
