...
odcore::data::dmcp::ModuleExitCodeMessage::ModuleExitCode DetectCone::body()
{
  ...
  while(getModuleStateAndWaitForRemainingTimeInTimeslice() == odcore::data::dmcp::ModuleStateMessage::RUNNING)
  {
    ...
    if (detectedConesLeft.rows()>1 && detectedConesRight.rows()>1) {
      ...
      if (index2>globalPath.size()-3)
        index2 = 0;
      if (diff>=3 && ((lastClosestPointIndex < closestPointIndex && diff<100.0f) || (lastClosestPointIndex > closestPointIndex && diff>100.0f)) ) {
