...
void BlackBox::generateSurfaces(ArrayXXf sideLeft, ArrayXXf sideRight, ArrayXXf location){
  ...
  float const coneWidthSeparationThreshold = kv.getValue<float>("sim-cfsd18-cognition-blackbox.coneWidthSeparationThreshold");
  float const guessDistance = kv.getValue<float>("sim-cfsd18-cognition-blackbox.guessDistance");

  ArrayXXf orderedConesLeft = BlackBox::orderAndFilterCones(sideLeft,location);
  ArrayXXf orderedConesRight = BlackBox::orderAndFilterCones(sideRight,location);

  float pathLengthLeft = BlackBox::findTotalPathLength(orderedConesLeft);
  float pathLengthRight = BlackBox::findTotalPathLength(orderedConesRight);
  ...
  if(leftIsLong)
  {
    ...
    ArrayXXf tmpShortSide = BlackBox::insertNeededGuessedCones(orderedConesLeft, orderedConesRight, location, coneWidthSeparationThreshold,  guessDistance, false);
  ...
  else
  {
    ...
    ArrayXXf tmpShortSide = BlackBox::insertNeededGuessedCones(orderedConesRight, orderedConesLeft, location, coneWidthSeparationThreshold,  guessDistance, true);
  ...
  if(longSide.rows() > 1)
  {
    ...
    BlackBox::findSafeLocalPath(longSide, shortSide);
  }
  else
  {
    if(longSide.rows() == 0)
...
void BlackBox::findSafeLocalPath(ArrayXXf sidePointsLeft, ArrayXXf sidePointsRight)
{
  ArrayXXf longSide, shortSide;

  // Identify the longest side
  float pathLengthLeft = BlackBox::findTotalPathLength(sidePointsLeft);
  float pathLengthRight = BlackBox::findTotalPathLength(sidePointsRight);
  ...
  ArrayXXf virtualPointsLong = BlackBox::placeEquidistantPoints(longSide,true,nMidPoints,-1);
  ...
  for(int i = 0; i < nMidPoints; i = i+1)
  {
    ...
    if(closestConeIndex == 0)
    {
      ...
      if(shortSide.rows() > 1)
      {
        factor = BlackBox::findFactorToClosestPoint(shortSide.row(0),shortSide.row(1),virtualPointsLong.row(i));
      }
    ...
    {
      factor = BlackBox::findFactorToClosestPoint(shortSide.row(nConesShort-2),shortSide.row(nConesShort-1),virtualPointsLong.row(i));
    ...
    {
      factor = BlackBox::findFactorToClosestPoint(shortSide.row(closestConeIndex-1),shortSide.row(closestConeIndex),virtualPointsLong.row(i));
      if(factor > 0 && factor <= 1)
      {
      ...
      {
        factor = BlackBox::findFactorToClosestPoint(shortSide.row(closestConeIndex),shortSide.row(closestConeIndex+1),virtualPointsLong.row(i));
        if(factor > 0 && factor <= 1)
        {
  ...
  BlackBox::sendMatchedContainer(virtualPointsLongFinal, virtualPointsShortFinal);
...
ArrayXXf BlackBox::placeEquidistantPoints(ArrayXXf sidePoints, bool nEqPointsIsKnown, int nEqPoints, float eqDistance)
{
...
ArrayXXf BlackBox::traceBackToClosestPoint(ArrayXXf p1, ArrayXXf p2, ArrayXXf q)
{
...
ArrayXXf BlackBox::orderCones(ArrayXXf cones, ArrayXXf vehicleLocation)
{
...
ArrayXXf BlackBox::orderAndFilterCones(ArrayXXf cones, ArrayXXf vehicleLocation)
{
  // Input: Cone and vehicle positions in the same coordinate system
  ...
  auto kv = getKeyValueConfiguration();
  float const coneLengthSeparationThreshold = kv.getValue<float>("sim-cfsd18-cognition-blackbox.coneLengthSeparationThreshold");
  float const maxConeAngle = kv.getValue<float>("sim-cfsd18-cognition-blackbox.maxConeAngle");
...
ArrayXXf BlackBox::insertNeededGuessedCones(ArrayXXf longSide, ArrayXXf shortSide, ArrayXXf vehicleLocation, float distanceThreshold, float guessDistance, bool guessToTheLeft)
{
  ...
  for(int i = 0; i < nConesLong; i = i+1)
  {
    ...
    if(shortestDist > distanceThreshold && longSide.rows()>1)
    {
      if(i == 0)
      {
        guess = BlackBox::guessCones(longSide.row(0),longSide.row(1),guessDistance,guessToTheLeft,true,false);
        nGuessedCones = nGuessedCones+1;
      }
      else if(i == nConesLong-1)
      {
        guess = BlackBox::guessCones(longSide.row(nConesLong-2),longSide.row(nConesLong-1),guessDistance,guessToTheLeft,false,true);
        nGuessedCones = nGuessedCones+1;
      }
      else
      {
        guess = BlackBox::guessCones(longSide.row(i-1),longSide.row(i),guessDistance,guessToTheLeft,false,true);
        nGuessedCones = nGuessedCones+1;
        guessedCones.row(nGuessedCones-1) = guess;
        guess = BlackBox::guessCones(longSide.row(i),longSide.row(i+1),guessDistance,guessToTheLeft,true,false);
        nGuessedCones = nGuessedCones+1;
      } // End of else
  ...
  ArrayXXf newShortSide = BlackBox::orderCones(realAndGuessedCones,vehicleLocation);
  return newShortSide;
} // End of insertNeededGuessedCones


ArrayXXf BlackBox::guessCones(ArrayXXf firstCone, ArrayXXf secondCone, float guessDistance, bool guessToTheLeft, bool guessForFirstCone, bool guessForSecondCone)
{
...
float BlackBox::findTotalPathLength(ArrayXXf sidePoints)
{
...
float BlackBox::findFactorToClosestPoint(ArrayXXf p1, ArrayXXf p2, ArrayXXf q)
{
...
void BlackBox::sendMatchedContainer(Eigen::ArrayXXf virtualPointsLong, Eigen::ArrayXXf virtualPointsShort)
{
